...
struct Person PEOPLE[] = {
    {"101001", "Faith", "Ian", 80000, 10},        {"102001", "Tufnel", "Nigel", 90000, 12},
    {"103001", "Savage", "Viv", 50000, 4},        {"104001", "Shrimpton", "Mick", 50000, 4},
    {"105001", "Besser", "Joe", 40000, 1},        {"106001", "Smalls", "Derek", 80000, 10},
    {"107001", "St.Hubbins", "David", 90000, 12}, {"108001", "Fleckman", "Bobbi", 120000, 8},
    {"109001", "Eton-Hogg", "Denis", 250000, 22}, {"110001", "Upham", "Denny", 60000, 5},
    {"111001", "McLochness", "Ross", 60000, 5},   {"112001", "Pudding", "Ronnie", 50000, 2},
    {"113001", "Schindler", "Danny", 60000, 3},   {"114001", "Pettibone", "Jeanine", 85000, 3},
    {"115001", "Fame", "Duke", 95000, 8},         {"116001", "Fufkin", "Artie", 45000, 1},
    {"117001", "DiBergi", "Marty", 15000, 7},     {"118001", "Floyd", "Pink", 25000, 6},
    {"119001", "Zeppelin", "Led", 35000, 3},      {"120001", "Mason", "Nick", 15000, 7},
    {"121001", "Wright", "Richard", 17000, 9},    {"122001", "Waters", "Roger", 10000, 3},
...
ostream& operator<<(ostream& os, const Person& person)
{
    ...
    sprintf(
        buffer,
        "Key: %8s, Name: %12s, %12s    Salary:%7.0f, Years: %2d",
        person.ID,
        person.lastName,
        person.firstName,
        person.salary,
        ...
        if (slot->State == OAHashTable<T>::OAHTSlot::OCCUPIED)
        {
            if (!shf)
                sprintf(
                    ...
                    i,
                    slot->Key,
                    phf(slot->Key, ht.GetStats().TableSize_));
            else
                sprintf(
                    buffer,
                    "Slot: %3d, Key: %s (%d:%d)\n",
...
void TestALot(HashData* phd, HashData* shd)
{
    ...
    typedef Person* T;
    OAHashTable<T> ht(OAHashTable<T>::OAHTConfig(
        initial_size, phf, shf, max_load_factor, growth_factor, policy, 0));
    DumpTable<T>(ht);
    DumpStats<T>(ht);
    cout << endl;
    ...
    try
    {
        ...
        key = keys[0];
        cout << endl
             << "Finding key: " << key << " (" << phf(key, ht.GetStats().TableSize_) << ")" << endl;
        try
        {
        ...
        {
            cout << endl
                 << "**** Something bad happened finding key " << key << " in TestALot" << endl
                 << endl;
        }
        DumpStats<T>(ht);
...
void TestDoubleHashing(HashData* phd, HashData* shd)
{
    ...
    typedef Person* T;
    OAHashTable<T> ht(OAHashTable<T>::OAHTConfig(
        initial_size, phf, shf, max_load_factor, growth_factor, MARK, 0));
    DumpTable<T>(ht);
    DumpStats<T>(ht);
    cout << endl;
    ...
    try
    {
        ...
        for (unsigned i = 0; i < size; i++)
        {
            Person* person = PersonRecs[i];
            key = person->ID;
            cout << "Inserting key: " << key << " (" << phf(key, ht.GetStats().TableSize_) << ":"
                 << (shf(key, ht.GetStats().TableSize_ - 1) + 1) << ")" << endl;
            ht.insert(person->ID, person);
        }
        DumpTable<T>(ht);
        DumpStats<T>(ht);
        ...
        key = person->ID;
        cout << endl
             << "Inserting key: " << key << " (" << phf(key, ht.GetStats().TableSize_) << ":"
             << (shf(key, ht.GetStats().TableSize_ - 1) + 1) << ")" << endl;
        ht.insert(person->ID, person);
    ...
    {
        cout << endl
             << "**** Something bad happened inserting in TestDoubleHashing" << endl
             << endl;
    }
}
...
void TestSimpleInsert1()
{
    ...
    {
        cout << endl
             << "**** Something bad happened inserting in TestSimpleInsert1" << endl
             << endl;
    }
}
...
void TestSimpleInsert2()
{
    ...
    {
        cout << endl
             << "**** Something bad happened inserting in TestSimpleInsert2" << endl
             << endl;
    }
}
...
void TestSimpleDeletePresent()
{
    ...
    try
    {
        ...
        {
            cout << endl
                 << "**** Something bad happened finding key " << key
                 << " in TestSimpleDeletePresent" << endl
                 << endl;
        }
    ...
    {
        cout << endl
             << "**** Something bad happened inserting in TestSimpleDeletePresent" << endl
             << endl;
    }
}
...
void TestSimpleDeleteMissing()
{
    ...
    try
    {
        ...
        catch (...)
        {
            cout << endl
                 << "**** Something bad happened finding key " << key
                 << " in TestSimpleDeleteMissing" << endl
                 << endl;
        }
        ht.remove(key);
    }
    ...
    {
        cout << endl
             << "**** Something bad happened inserting in TestSimpleDeleteMissing" << endl
             << endl;
    }
    DumpStats<T>(ht);
}
...
void TestSimpleDispose()
{
    ...
    for (unsigned i = 0; i < 11; i++)
    {
        ...
        {
            cout << endl
                 << "**** Something bad happened inserting in TestSimpleDispose" << endl
                 << endl;
            break;
        }
    }
...
void TestSimpleGrow1()
{
    ...
    typedef char* T;
    OAHashTable<T> ht(
        OAHashTable<T>::OAHTConfig(5, SimpleHash, NULL, lf, 2.0, PACK, FreeSimpleRec));
    for (unsigned i = 0; i < 30; i++)
    {
      ...
      DumpStats<T>(ht);
        ...
        {
            cout << endl
                 << "**** Something bad happened inserting in TestSimpleGrow1" << endl
                 << endl;
            break;
        }
    }
...
void TestSimpleGrow2()
{
    ...
    for (unsigned i = 0; i < 30; i++)
    {
      ...
      DumpStats<T>(ht);
        ...
        {
            cout << endl
                 << "**** Something bad happened inserting in TestSimpleGrow2" << endl
                 << endl;
            break;
        }
    }
...
void TestSimpleMarkPack(HashData* phd, HashData* shd, OAHTDeletionPolicy policy)
{
    ...
    catch (OAHashTableException& e)
    {
        ...
        // cout << endl << "errno: " << e.code() << ", " << e.what() << endl << endl;
        // cout << endl << "**** Caught exception deleting non-existent item. (SimpleTest1) ****" <<
        // endl << endl;
    }
    catch (...)
    {
...
/*
  Why are the hashes so different when the same function is used for
  both primary and secondary hash? e.g. TableSize is 13:

  57 % 13 = 5
  57 % 12 = 9

  The modulo can vary significantly with a difference in 1 by
  the table size.
...
int main(int argc, char** argv)
{
    ...
    // OAHashTable<float *>::OAHTConfig config(11, ConstantHash);
    // OAHashTable<float *> ht(config); // <----if this is changed to <float *> the program
